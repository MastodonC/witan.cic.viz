...
cic_placement_age_group <- function(input_dir, output_dir, from_date, to_date, group_ages) {
  ...
  cic_by_age_pdf <- "cic-placement-age.pdf"
  ...
  chart_data <- episodes %>%
    dplyr::inner_join(data.frame(month = floor_date(seq(month_start(from_date), month_start(to_date), by = "month"), unit = "month")), by = character()) %>%
    dplyr::filter(start <= month & end >= month) %>%
    dplyr::mutate(age = year_diff(birthday, month)) %>%
    dplyr::group_by(month, simulation, period_id) %>%
    dplyr::slice(1) %>%
    dplyr::group_by(month, age, placement, simulation) %>%
    dplyr::summarise(n = n_distinct(period_id), .groups = "drop_last") %>%
    dplyr::summarise(n = median(n))
  
  write.csv(chart_data, file = file.path(output_dir, "cic-placement-age.csv"), row.names = FALSE)
  ...
  for (the.placement in placements) {
    print(ggplot(chart_data %>% filter(placement == the.placement), aes(month, n, fill = factor(age))) +
            geom_bar(stat = "identity") +
            labs(x = "Month", y = "Children in care", fill = "Age", title = paste("Children in", the.placement)) +
            scale_fill_manual(values = tableau_color_pal("Tableau 20")(20))
    )
    print(ggplot(chart_data %>% filter(placement == the.placement), aes(month, n, fill = factor(age))) +
            geom_bar(stat = "identity", position = "fill") +
  ...
  for (age in age_labels) {
    print(ggplot(chart_data %>% filter(age == age), aes(month, n, fill = placement)) +
            geom_bar(stat = "identity") +
            labs(x = "Month", y = "Children in care", fill = "Placement", title = paste(age, "children")) +
            scale_fill_manual(values = placement.colours)
    )
